...
for iter4 = 1:length(zoom)
    for iter1 = [2,4] %1:length(freqs)
        ...
        input_y = squeeze(cycle_positions(:,1,2));
        ...
        select_frames = round(linspace(1,cycle/2+1,num_selected));
        iter0 = 0;
        for iter2 = 1:cycle
            fig = figure;
            ...
            if ismember(iter2, select_frames)
                ...
                plot(input_y)
                hold on;
                color_idx = min([max([round((input_y(iter2)+1)*256/2),1]),256]);
                plot(iter2,input_y(iter2),'.','MarkerSize',dot_size,'Color',squeeze(color_map(color_idx,:)))
                xlim([0 80]);
                hold off;
