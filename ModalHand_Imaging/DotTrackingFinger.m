...
for iter4 = 1:length(zoom)
    for iter1 = 1:length(freqs)
        ...
        file_name = strcat(zoom(iter4),"_",dots,"_",num2str(freqs(iter1)),"Hz_",num2str(frame_rate),"fps.tif");
        ...
        if zoom(iter4) == "Zoom1"
            ...
            point_locations = maskDots(points,30,cut_x,cut_y,cut_thickness);
        end
        if zoom(iter4) == "Zoom2"
            points = detectKAZEFeatures(objectFrame(:,:,1),'ROI',objectRegion,'Threshold',.004,'NumOctaves',2);
...
save ImageData.mat zoom freqs frame_rate tracking_cell displacement_cell
