function [Fz] = Spatial_Loop(MeasurementSignal, MeasurementInfo, TestSignal, freq, amp, trial_num, daq_in)
    ...
    while ~stable_force
        ...
        plotTrialNum(mean_force,desired_preload,"Apply Constant Force",[.94, .94 .94],trial_num,1)
        if and(min_force > desired_preload(1), max_force < desired_preload(2))
            stable_force = true;
        end
    end
    
    plotTrialNum(mean_force,desired_preload,"Playing Vibration",'b',trial_num,1)
