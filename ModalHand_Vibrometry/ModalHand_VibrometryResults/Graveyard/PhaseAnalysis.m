...
start_idx = 2;%6000;
width_idx = 20;
disp(highRes.freq(freq_idx(start_idx+width_idx)));
...
for iter1 = freq_idx
    ...
    if and(iter1>start_idx, iter1<start_idx+width_idx)
        for iter3 = 1:3
            subplot(1,2,1)
            plot(position(iter3:3:end),unwrap(fixed_phase(iter0,iter3:3:end)-fixed_phase(iter0,1)),'Color',colormap(iter1-start_idx,:));
            hold on;
            subplot(1,2,2)
            plot(position(iter3:3:end),unwrap(free_phase(iter0,iter3:3:end)-free_phase(iter0,1)),'Color',colormap(iter1-start_idx,:));
            hold on;
        end
    end
end
