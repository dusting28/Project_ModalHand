...
sig_length = 1;
% noise_sig = rand(1,sig_length*fs)-.5;
noise_sig = 1;

% Create ensemble matrix from IRs
if strcmp(hand_condition,"Free")
    input_ensemble = zeros(length(highRes.free_ir{1})+length(noise_sig)-1,length(highRes.yCord)-not(include_probe));
    for iter1 = 1:(length(highRes.yCord)-not(include_probe))
        pos_idx = iter1 + not(include_probe);
        input_signal = conv(highRes.free_ir{pos_idx},noise_sig);
...
if strcmp(hand_condition,"Fixed")
    ...
    for iter1 = 1:(length(highRes.yCord)-not(include_probe))
        pos_idx = iter1 + not(include_probe);
        input_signal = conv(highRes.fixed_ir{pos_idx},noise_sig);
        input_ensemble(:,iter1) = input_signal- mean(input_signal);
    end
end
...
figure;
contribution_t = zeros(ceil(size(V,2)/2)+1,num_modes);
for iter1 = 1:num_modes
...
figure;
contribution_s = zeros(ceil(size(V,2)/2)+1,num_modes);
for iter1 = 1:num_modes
...
%Plot Input Signal
t = (0:length(noise_sig)-1)/fs;
t_up = (0:0.001:length(noise_sig)-1)/fs;
t_zero = t(1);
samp_idx = 31+5*(1:10);
% noise_up = spline(t,noise_sig,t_up);
% figure;
% plot(t_up-t_zero,noise_up);
% hold on;
% plot(t(samp_idx)-t_zero,noise_sig(samp_idx),'.');
% hold off;
...
for iter1 = 1:num_modes
    figure;
    signal = -movmean(real(V(iter1,:)),kernal);
    plot(t_out-t_zero,signal);
    hold on;
    plot(t_out(samp_idx)-t_zero,signal(samp_idx),'.');
    hold off;
...
for iter1 = 1:num_modes
    ...
    waterfallPlot2(single_axis(370:2:410,:),0,3,highRes,flipud(colorcet('COOLWARM')),include_probe);
