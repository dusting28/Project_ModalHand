...
%----------------------------------------------------------
plot = false;
refLength = 50000; %add extra signal length for reference signal 
endLength = 20000; %extra time at end of signal for decay
% refLength = 1;
% endLength = 1;
...
for i = 1:length(refInd)
    yOut(refInd(i):refInd(i)+500,nActuators+1) = refVoltage; % one reference pulse for each actuator
end
