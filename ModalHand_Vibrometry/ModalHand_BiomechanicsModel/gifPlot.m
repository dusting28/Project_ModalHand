function [] = gifPlot(data,K,highRes,c_lim,colors,fig_title,include_probe)
    ...
    y_line = linspace(highRes.yCord(2),highRes.yCord(end),1000);
    z_line = 0;

    hFigure = figure;
    set(hFigure, 'MenuBar', 'none');
    set(hFigure, 'ToolBar', 'none');
    s = surf(xMesh,yMesh,zMesh,'Visible', 'on');
    s.EdgeColor = 'none';
    colormap(colors);
    ...
    title(fig_title)
    hold on;
    plot3(x_line,highRes.yCord(2)*ones(1,size(x_line,2))',z_line*ones(1,size(x_line,2)),'Color','black','LineWidth',1)
    hold on;
    plot3(x_line,highRes.yDIP*ones(1,size(x_line,2))',z_line*ones(1,size(x_line,2))','Color','black','LineWidth',1)
    hold on;
    plot3(x_line,highRes.yPIP*ones(1,size(x_line,2)),z_line*ones(1,size(x_line,2)),'Color','black','LineWidth',1)
    hold on;
    plot3(x_line,highRes.yMCP*ones(1,size(x_line,2)),z_line*ones(1,size(x_line,2)),'Color','black','LineWidth',1)
    hold on;
    plot3(x_line,highRes.yCord(end)*ones(1,size(x_line,2)),z_line*ones(1,size(x_line,2)),'Color','black','LineWidth',1)
    hold on;
    plot3(highRes.xCord(2)*ones(1,size(y_line,2)),y_line,z_line*ones(1,size(y_line,2)),'Color','black','LineWidth',1)
    hold on;
    plot3(highRes.xCord(4)*ones(1,size(y_line,2)),y_line,z_line*ones(1,size(y_line,2)),'Color','black','LineWidth',1)
    hold off;
    ...
    %axis off
    set(gca, 'color', 'none');
    exportgraphics(gcf,strcat(fig_title,".gif"),'BackgroundColor','none','Append',true);
    close(hFigure) 
end